...
LMML_LAT, LMML_LONG = 35.857, 14.477  # Malta International AirportW
...
class TestAlpha(core.Entity):
    ...
    def init_planes(self):
        for plane_id in traf.id:
            ...
            # destination = DESTINATIONS.get(name.upper(), [0, 0])
            # print(type(lat), type(lon), type(destination[0]), type(destination[1]))
            # distance_to_destination = geodesic((lat, lon), (destination[0], destination[1])).km
            # print(f"{name} : {lat}, {lon} | {destination[0]}, {destination[1]} -> {distance_to_destination:.2f} km")
    ...
    def action_apply_action(self, actions):
        ...
        for plane_id, action in actions.items():
            if plane_id in self.planes:
                current_heading = self.planes[plane_id].heading
                new_heading = (current_heading + action) % 360
                # traf.hdg[traf.id2idx(plane_id)] = new_heading
    ...
    def action_reset(self):
        ...
        stack.stack(f"OPEN alpha/train/train_0011")
        self.init_planes()
